...
std::optional<int64_t> GetConstantIntValue(Value v) {
  ...
  if (def && def->getName().getStringRef().ends_with("arith.constant")) {
    if (auto int_attr = def->getAttrOfType<mlir::IntegerAttr>("value")) {
...
LogicalResult delinearize_subcore(Operation* op, int core_id_seg,
  ...
  if (auto core_int = GetConstantIntValue(core_id_val)) {
    int64_t val = *core_int;
    new_core_id = create_constant_op(loc, val / num_subcores);
    new_subcore_id = create_constant_op(loc, val % num_subcores);
  }
  if (!new_core_id) {
    Value c16 = create_constant_op(loc, num_subcores);
    new_core_id = builder.create<arith::DivUIOp>(loc, core_id_val, c16);
    new_subcore_id = builder.create<arith::RemUIOp>(loc, core_id_val, c16);
  }
...
LogicalResult linearize_subcore(Operation* op, int core_id_seg,
  ...
  if (auto core_int = GetConstantIntValue(core_id_val)) {
    if (auto subcore_int = GetConstantIntValue(subcore_id_val)) {
      int64_t val = *core_int * num_subcores + *subcore_int;
      new_core_id = create_constant_op(loc, val);
    }
  }
  if (!new_core_id) {
    Value c16 = create_constant_op(loc, 16);
    ...
    new_core_id = add_op->getResult(0);
  }
  op->setOperand(core_id_op_idx, new_core_id);
...
LogicalResult wait_dma2_upgrade(Operation* op, int version, bool&) {
  ...
  if (version < 14) {
    ...
    if (new_sizes.size() != 5) {
      return op->emitError(
          "Unexpected size of AttrSizedOperandSegments for wait_dma2");
    }
    new_sizes.push_back(0);
    op->setAttr(OpTrait::AttrSizedOperandSegments<
                    EnqueueDMAOp>::getOperandSegmentSizeAttr(),
                DenseI32ArrayAttr::get(op->getContext(), new_sizes));
    return delinearize_subcore(op, 4, 5);
  }
  return success();
}
...
LogicalResult wait_dma2_downgrade(Operation* op, int version, bool&) {
  if (version < 14) {
    ...
    if (sizes[5] != 0) {
      LogicalResult linearize = linearize_subcore(op, 4, 5);
      if (linearize.failed()) {
        return linearize;
      }
    }
